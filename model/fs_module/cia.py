...
class CIA(nn.Module):
    ...
    def forward(self,sample_inpt):
        support, query = sample_inpt[:self.k*self.n], sample_inpt[self.k*self.n:]
        support = support.reshape(self.k, self.n, -1)
        prototype = torch.mean(support, dim=1)
        sample_inpt = torch.cat((prototype, query))
        feat=self.self_interaction(sample_inpt)
        proto_feat,query_feat =self.cross_fusion(feat)

        dist = self.get_dist(proto_feat, query_feat)
